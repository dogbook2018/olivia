...
$query = $db->query("SELECT * FROM pages ORDER BY pages_id DESC LIMIT ".$start_from.",". $limit);
...
<?php require_once('../function/backend_nav.php'); ?>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  <a href="delete.php?pages_id=<?php echo $pages['pages_id']; ?>" class="btn btn-outline-primary" onclick="if(!confirm('是否確定刪除此筆資料?刪除後無法回復')){return false;};">刪除</a>
                </td>
              </tr>
              <?php } ?>
            </tbody>
  ...
  <pingendo onclick="window.open('https://pingendo.com/', '_blank')" style="cursor:pointer;position: fixed;bottom: 10px;right:10px;padding:4px;background-color: #00b0eb;border-radius: 8px; width:250px;display:flex;flex-direction:row;align-items:center;justify-content:center;font-size:14px;color:white">Oliva House
